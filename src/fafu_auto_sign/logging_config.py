...
class NotificationHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        try:
            ...
            # 检查签到成功标记
            if "✅ 签到成功" in message:
                self.notification_service.notify(title="签到成功", content=message, success=True)
            # 检查签到失败标记
            elif "❌ 签到失败" in message:
                self.notification_service.notify(title="签到失败", content=message, success=False)
            # 检查Token过期标记（紧急通知）
            elif "[x] Token已过期" in message or "Token 已过期" in message:
                self.notification_service.notify(
                    title="紧急：Token已过期", content=message, success=False
                )
        except Exception:
            # 通知处理不应影响日志记录，静默处理异常
...
def setup_logging(
    ...
    # 日志配置完成
    logging.getLogger(__name__).debug(f"日志已配置: 控制台={log_level}, 文件=DEBUG, 目录={log_dir}")
