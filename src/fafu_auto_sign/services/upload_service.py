...
class UploadService:
    ...
    def _get_latest_image(self, image_dir: str) -> Optional[str]:
        """从目录中选择最新修改的图片。

        扫描指定目录，过滤隐藏文件和不支持的扩展名，
        返回最新修改的图片文件的完整路径。
        ...
        返回:
            最新修改的图片文件路径，如果目录为空或不包含支持的图片则返回 None。
        """
        ...
        self.logger.info(f"[*] 选择最新修改的图片: {Path(selected).name}")
        return selected
    ...
    def upload_image(self, image_path: str) -> Optional[str]:
        ...
        if hasattr(self.client, "config") and self.client.config.latest_image_dir:
            latest_image_dir = self.client.config.latest_image_dir
            self.logger.info(f"[*] 使用最新图片目录: {latest_image_dir}")

            selected = self._get_latest_image(latest_image_dir)

            if selected is None:
                self.logger.error(
                    f"[x] 错误：最新图片目录为空或不包含支持的图片格式: {latest_image_dir}"
                )
                return None
        ...
        try:
            ...
            with open(actual_image_path, "rb") as f:
                files = {"file": (os.path.basename(actual_image_path), f, "image/jpeg")}
